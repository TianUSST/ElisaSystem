...
- **深度学习模型支持**：支持U-Net、Attention U-Net、R2U-Net、Swin-Unet和FCN等多种模型架构
...
- **双线程队列处理**：采用多线程和队列机制，确保分割和分类任务有序执行
- **批量检测流水线**：支持从图像分割到阳性孔分类的完整批量处理流程
- **检测结果导出**：支持导出详细的联通区域分析信息和标准化96孔板格式结果
...
2. 选择模型和权重文件：
   - 通过菜单栏选择模型类型（U-Net、Attention U-Net等）
   - 选择对应的预训练权重文件
...
6. 批量检测流水线：
   ...
   - 设置保存路径
   - 勾选需要保存的结果选项
   - 点击"开始批量检测"按钮，系统将自动完成从图像分割到阳性孔分类的完整流程

7. 导出检测结果：
   - 在批量处理时勾选"导出详细信息"和"导出板式格式"选项
   - 系统将自动生成对应的CSV文件
...
为了提高处理效率并确保结果的正确性，系统采用了双线程队列处理机制：

1. **分割线程**：负责图像分割任务，使用深度学习模型对图像进行处理
2. **分类线程**：负责阳性/阴性分类任务，对分割出的ROI区域进行分类
3. **处理队列**：确保分割和分类任务按顺序执行，避免并发访问导致的数据竞争
4. **结果保存**：自动保存处理结果到指定目录

该机制有效解决了分割速度快于分类速度可能导致的问题，确保每个图像的分割和分类任务都能正确完成。

### 批量检测流水线

系统支持完整的批量检测流水线，可以自动完成以下步骤：
...
用户只需选择图像目录和保存路径，系统即可自动处理所有图像，并将结果保存到指定目录。
